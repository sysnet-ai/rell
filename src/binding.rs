...
impl BindingState
{
    ...
    // Compatible Mappings:
    // We say the binding variable states are compatible if all the instances of a
    // variables (i.e. 'X') points to the same literal AND
    // No two different variables point to the same literal.
    //
    // for example if we have the following statements in our Binding State:
    // a.X and b.X
    //
    // The mappings generated by the binding code will only be considered compatible
    // if X points to the same literal in both cases.
    //
    // Similarly if we have the following statements:
    // a.Y and b.X
    // The bindings will only be considered compatible if X and Y point to different
    // literals.
    //
    // TODO: It could(?) should(?) be possible for numeric variables to repeat (?)
    ...
    // Bind Statement To Tree Algorithm:
    //
    // Do a prunable breadth first traversal starting from the tree root
    //
    // For each symbol that was parsed:
    //    While there are nodes to traverse in the visit list:
    //      if the symbol is an identifier:
    //         add all the children of the current node in the traversal to the future visit list,
    //         and add the Variable <> Literal to the bound variables 
    //      else:
    //          Only add the child that matches the symbol in the parsed statement
    //
    //
    // Example:
    //
    ...
    //                .d.e
    //
    //    Current Node starts = ROOT, statement symbol = Literal(a), visit list []
    //       Since it is literal - add to the visit list [visit list Node(a)]
    //       <loop>
    //
    //    Current Node = a, statement symbol = Identifier(X), visit list []
    //       Since it is an ID - add all the children to the visit list and add the bounded var
    //       visit list = [(Node(b), {X: b}), (Node(d), {X: d})] 
    // 
    //    No more symbols so return the bindings: X=b and X=d
    //
    //    if the Statement was a.X.e
    //    Then on the last loop - The visit list would be [(Node(e), {X: d})] which is the only
    //    valid binding for X to fulfill the statement.
    //
    //TODO:
    //  It's weird that we're keeping the Path and the NID in the variable state even if we don't
    //  actually "need" them.
    //  The path is good for self.get_all_bound_paths_for(...) but seems a bit odd.
    //  NID is only used during the traversal, while technically it will return the node where the
    //  binding ended up being successful - but what's the use for it?
    //
    //
...
mod test
{
    ...
    fn test_binding_state() -> Result<()>
    {
        ...
        let compatible_var_bindings = bs.generate_compatible_on(&w);
        ...
        w.symbols.bind_variables(&compatible_var_bindings[0]);
    ...
    fn empty_binding() -> Result<()> 
    {
        let w = build_test_tree()?;
        let mut bs = BindingState::new();
        bs.add_statement("X.plus.Y");

        let compatible_var_bindings = bs.generate_compatible_on(&w);
        assert_eq!(compatible_var_bindings.len(), 0, "Incorrect length for empty binding expected");
